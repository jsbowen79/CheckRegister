...
describe('Transaction Class functions', () => {
    ...
    beforeEach(() => {
        ...
        transaction = new Transaction(100.5, TransMedia.Cash, 'Test', findCategoryNode(root, CategoryNames.Employment), 1, TransType.Credit, 123, date, 100.25, Status.Complete);
    });
    test('Transaction Object Complete', () => {
        ...
        expect(transaction.category).toBe(findCategoryNode(root, CategoryNames.Employment));
    ...
    test('Determines status of Deposits correctly', () => {
        let result = transaction.determineStatus();
        expect(transaction.status).toBe(Status.Complete);
        ...
        result = transaction.determineStatus();
        expect(result).toBe('Transaction Pending.  Please allow 24 hours for review. Cash Deposits are credited upon review.' +
